 ...
 * @author chenyian
 */
public class TmHomologeneConverter extends BioFileConverter {
	private static final Logger LOG = LogManager.getLogger(TmHomologeneConverter.class);
	//
	private static final String DATASET_TITLE = "HomoloGene";
	 ...
	 *            the Model
	 */
	public TmHomologeneConverter(ItemWriter writer, Model model) {
	...
	public void process(Reader reader) throws Exception {
		...
		while (iterator.hasNext()) {
			...
			if (taxonIds.contains(cols[1])) {
				String key = cols[0];
				if (clusterMap.get(key) == null) {
					clusterMap.put(key, new HashSet<String>());
				}
				clusterMap.get(key).add(String.format("%s_%s", cols[1], cols[2]));
			}
		}
