...
/**
 * NOTE: borrow from InterMine go-annotation source
...
public class GoGoslimPostprocess extends PostProcessor
{
     ...
     */
    public GoGoslimPostprocess(ObjectStoreWriter osw) {
        super(osw);
    ...
    public void postProcess() throws ObjectStoreException {
        ...
        while (resIter.hasNext()) {
            ...
            Set<GOEvidence> evidence = downcastEvidenceSet( thisAnnotation.getEvidence() );
    ...
    private void mergeEvidence(Set<GOEvidence> evidence, GOAnnotation alreadySeenAnnotation) {
        for (GOEvidence g : evidence) {
            GOEvidenceCode c = (GOEvidenceCode)g.getCode();
            Set<Publication> pubs = g.getPublications();
            boolean foundMatch = false;
            for (GOEvidence alreadySeenEvidence : downcastEvidenceSet( alreadySeenAnnotation.getEvidence() ) ) {
                GOEvidenceCode alreadySeenCode = (GOEvidenceCode)alreadySeenEvidence.getCode();
    ...
    private Set<GOEvidence> downcastEvidenceSet(Set<OntologyEvidence> ontologyEvidenceSet){
        Set<GOEvidence> goEvidenceSet = new HashSet<GOEvidence>();
        for( OntologyEvidence ontologyEvidence : ontologyEvidenceSet ){
            goEvidenceSet.add( (GOEvidence)ontologyEvidence );
        }
        return goEvidenceSet;
    }
