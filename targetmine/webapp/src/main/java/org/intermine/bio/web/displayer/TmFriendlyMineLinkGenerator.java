...
/**
 * A customized FriendlyMineLinkGenerator for TargetMine due to the different homolog model.  
 * Helper class for intermine links generated on report and lists pages
 *
 * @author Julie Sullivan
 */
public final class TmFriendlyMineLinkGenerator implements InterMineLinkGenerator
{
...
//    private static final Logger LOG = Logger.getLogger(TmFriendlyMineLinkGenerator.class);

    /**
     * Constructor
     */
    public TmFriendlyMineLinkGenerator() {
        super();
    }
    ...
    private class LinkFetcher
    {
        ...
        /**
         * customization for TargetMine (chenyian)
         */
		private PathQuery getLocalHomologueQuery(Mine mine, ObjectRequest req) {
			PathQuery q = new PathQuery(mine.getModel());
			q.addViews("Gene.proteins.orthologProteins.genes.primaryIdentifier",
		...
		// the loop constraint has some problems (2019/6/25)
//      private PathQuery getLocalHomologueQuery(Mine mine, ObjectRequest req) {
//			PathQuery q = new PathQuery(mine.getModel());
//			q.addViews("Gene.homology.genes.primaryIdentifier", "Gene.homology.genes.symbol",
//					"Gene.homology.genes.name", "Gene.homology.genes.organism.name");
//
//			q.addOrderBy("Gene.homology.genes.primaryIdentifier", OrderDirection.ASC);
//			q.addConstraint(Constraints.lookup("Gene", req.getIdentifier(), req.getDomain()));
//			q.addConstraint(Constraints.notEqualToLoop("Gene.homology.genes", "Gene"));
//          return q;
//      }
